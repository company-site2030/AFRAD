...
if(document.getElementById('registerForm')){
  document.getElementById('registerForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      await setDoc(doc(db,'users',userCredential.user.uid),{fullName,phone,nationalId,email,createdAt:new Date()});
...
onAuthStateChanged(auth,async(user)=>{
  if(user){
    ...
    if(userDoc.exists()){
      ...
      document.getElementById('userInfo').innerHTML=`<h3>بيانات العميل</h3>
    ...
    document.getElementById('walletInfo').innerHTML=`<h3>المحفظة والرصيد</h3><p><strong>الرصيد:</strong>${balance}</p>`;
    document.getElementById('withdrawInfo').innerHTML=`<h3>السحب عبر البنوك المحلية</h3>
      <select id="bankSelect"><option>البنك الأهلي</option><option>مصرف الراجحي</option>
      <option>البنك السعودي الفرنسي</option><option>البنك السعودي للاستثمار</option>
      <option>البنك العربي الوطني</option><option>بنك الرياض</option></select>
